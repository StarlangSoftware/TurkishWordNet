...
public class WordNet1969Test extends PreviousWordNetTest{
    ...
    public void testSize() {
        assertEquals(38121, previousWordNet.size());
    }

    @Test
    public void testSynSetIdCounts() {
        CounterHashMap<String> counts = synSetIdCounts();
        assertEquals(2659, (int) counts.get("TUR01"));
        assertEquals(1146, (int) counts.get("TUR02"));
        assertEquals(346, (int) counts.get("TUR03"));
        assertEquals(153, (int) counts.get("TUR04"));
        assertEquals(702, (int) counts.get("TUR05"));
        assertEquals(33115, (int) counts.get("TUR10"));
    }
    ...
    public void testSynSetList() {
        ...
        assertEquals(48271, literalCount);
    }

    @Test
    public void testGetSynSetsWithPartOfSpeech() {
        assertEquals(20828, previousWordNet.getSynSetsWithPartOfSpeech(Pos.NOUN).size());
        assertEquals(8560, previousWordNet.getSynSetsWithPartOfSpeech(Pos.VERB).size());
        assertEquals(6124, previousWordNet.getSynSetsWithPartOfSpeech(Pos.ADJECTIVE).size());
        ...
        assertEquals(60, previousWordNet.getSynSetsWithPartOfSpeech(Pos.PRONOUN).size());
    }
    ...
    public void testLiteralList() {
        assertEquals(36876, previousWordNet.literalList().size());
    }

    @Test
    public void testLiteralWordCounts() {
        CounterHashMap<Integer> counts = literalWordCounts();
        assertEquals(26533, (int) counts.get(1));
        assertEquals(7970, (int) counts.get(2));
        assertEquals(1415, (int) counts.get(3));
        assertEquals(598, (int) counts.get(4));
